(() => {
  ...
  function time() { return new Date().toLocaleTimeString('sk-SK', { hour:'2-digit', minute:'2-digit' }); }
  function addMessage(text, user = false, fallback = false) {
    ...
    row.innerHTML = `${user ? '' : `<span class="message__avatar">${advisorLogo()}</span>`}<div><div class="bubble">${text}</div><small>${fallback ? 'Overená lokálna odpoveď · ' : ''}${time()}</small></div>`;
  ...
  function renderResult() {
    ...
    advisor.innerHTML = `<div class="result-head"><span>Osobné odporúčanie</span><h2>${esc(product.name)}</h2><p>Výber vychádza zo všetkých štyroch odpovedí.</p></div><section class="result-card"><div class="result-main">${productVisual(product)}<div class="result-main__copy"><span>${esc(product.price)}</span><h3>${esc(product.origin)}</h3><div class="taste-tags">${(product.tags||[]).map((tag)=>`<b>${esc(tag)}</b>`).join('')}</div></div></div><div class="result-details">${detailItem('Pre koho', product.bestFor || product.reason, 'home')}${detailItem('Do čoho', product.machines || 'Podľa zvoleného mletia', 'automatic')}${detailItem('Chuť', product.profile || (product.tags||[]).join(', '), 'smooth')}</div><div class="reason"><span>${icons.check}</span><div><b>Prečo práve táto</b><p>${esc(product.reason)}</p></div></div><div class="result-actions"><a class="result-button result-button--primary" href="${esc(product.url || config.shopUrl)}" target="_blank" rel="noreferrer">Pozrieť produkt ${icons.next}</a><button class="result-button result-button--secondary" id="restartResult" type="button">Zmeniť odpovede</button></div>${alternative ? `<div class="alternative"><div><small>Rozumná alternatíva</small><b>${esc(alternative.name)}</b><span>${esc(alternative.tags?.slice(0,2).join(' · ') || alternative.origin)}</span></div><button type="button" data-product="${esc(alternative.id)}">Zobraziť ${icons.next}</button></div>` : ''}${renderOfficeFollowup(product)}</section>`;
