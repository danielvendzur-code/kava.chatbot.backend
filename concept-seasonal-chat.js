(() => {
  ...
  function openWidget({ focus = false } = {}) {
    ...
    lockBackground();
    animateMarks('is-mark-entering');
    if (focus) setTimeout(() => $('#closeWidget').focus(), 260);
    emit('widget_open');
  }
  ...
  function closeWidget() {
    ...
    setTimeout(() => {
      ...
      $('#openWidget').focus({ preventScroll: true });
    }, 260);
  }
  ...
  function setMode(mode, { focusInput = false } = {}) {
    ...
    if (mode === 'chat' && focusInput && widget.classList.contains('is-open')) {
      setTimeout(() => $('#chatInput').focus({ preventScroll: true }), 230);
    }
    persist();
  ...
  function fallbackAnswer(text) {
    ...
    if (query.includes('mlie') || query.includes('capp') || query.includes('latte')) {
      ...
      return `Momentálne odpovedám v lokálnom režime. Do mlieka by som začal s <b>${escapeHTML(product.name)}</b>. ${escapeHTML(product.reason)}`;
    }
    if (query.includes('ovoc') || query.includes('filter') || query.includes('svie')) {
  ...
  async function requestAI(text) {
    ...
    const response = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ demoId: config.id, messages: app.state.chatHistory.slice(-8) })
    });
    if (!response.ok) throw new Error(`API ${response.status}`);
    const data = await response.json();
    if (!data.reply) throw new Error('Empty AI response');
    app.state.chatHistory.push({ role: 'assistant', content: data.reply });
    return data.reply;
  }
  ...
  async function sendChat(text, chip = null) {
    ...
    try {
      ...
      chip?.classList.remove('is-sending');
    }
  }
