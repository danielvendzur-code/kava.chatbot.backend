...
const optionSelector = '.pz-option:visible,.option:visible,.kf-option:visible,.advisor-option:visible,.choice-card:visible';
...
async function visiblePanel(page) {
  for (const selector of ['.kf-panel:visible', '#widget:visible', '.pz-widget:visible', '.widget:visible', '[role="dialog"]:visible']) {
    const locator = page.locator(selector).first();
...
async function openDemo(page, slug) {
  await page.goto(`${baseURL}/?demo=${slug}`, { waitUntil: 'networkidle' });
  await page.waitForTimeout(250);
  await page.screenshot({ path: `artifacts/final-matrix/${slug}-desktop-landing.png`, fullPage: true });
  await clickFirstVisible(page, ['#heroOpen', '#openWidget', '#launcherButton', '#launcher', '.launcher__button', '.pz-launcher']);
  await page.waitForTimeout(280);
  return visiblePanel(page);
}

async function selectMode(page, mode) {
  const selectors = mode === 'chat'
    ? ['[data-mode="chat"]:visible', '[data-view="chat"]:visible']
    : ['[data-mode="advisor"]:visible', '[data-view="advisor"]:visible'];
...
async function assertChat(page, panel, slug, mobile = false) {
  ...
  const input = panel.locator('input').filter({ visible: true }).last();
  await expect(input).toBeVisible();
  if (mobile) {
    expect(await page.evaluate(() => document.activeElement?.tagName === 'INPUT')).toBe(false);
  }

  const send = panel.locator('form button[type="submit"]:visible,.composer__send:visible,.pz-send:visible,.kf-send:visible').last();
  ...
  const chips = panel.locator(chipSelector);
...
async function finishAdvisor(page, panel, slug, mobile = false) {
  ...
  const loadedPhotoCount = await panel.locator('img:visible').evaluateAll((imgs) => imgs.filter((img) => img.complete && img.naturalWidth >= 80 && img.naturalHeight >= 80).length);
  expect(loadedPhotoCount).toBeGreaterThanOrEqual(1);
...
for (const [slug, brand] of demos) {
  test(`${brand} desktop director gate`, async ({ page }) => {
    ...
    const panel = await openDemo(page, slug);
    ...
    await assertChat(page, panel, slug, false);
    await finishAdvisor(page, panel, slug, false);
  ...
  test(`${brand} mobile director gate`, async ({ page }) => {
    ...
    await assertChat(page, panel, slug, true);
    await finishAdvisor(page, panel, slug, true);
    expect(errors).toEqual([]);
  });
}
