(() => {
  ...
  const { $, escapeHTML, productById, rankings, percentFor, persist, emit, animateMarks } = app;
    ...
    advisor.innerHTML = `
      ...
      <section class="result-card" aria-label="Odporúčaná káva ${escapeHTML(product.name)}">
        <div class="result-photo">
          ...
          <div class="result-photo__copy"><small>${escapeHTML(product.country)} · ${escapeHTML(product.process)}</small><h3>${escapeHTML(product.name)}</h3><span>${escapeHTML(product.tags.join(' · '))}</span></div>
        </div>
        <div class="result-body">
          ...
          <div class="result-story">
            ...
            <div class="story-block"><span>Dostupnosť</span><p>${escapeHTML(product.availability)}</p></div>
          </div>
          ${alternative ? `<button class="alternative" type="button" data-product="${escapeHTML(alternative.id)}"><span>Alternatíva</span><b>${escapeHTML(alternative.name)}</b><small>${escapeHTML(alternative.tags.slice(0, 3).join(' · '))}</small><em>${percentFor(alternative)} %</em></button>` : ''}
        </div>
      </section>`;
